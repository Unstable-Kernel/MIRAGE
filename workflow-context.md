...
1. `uv sync` (the documented first step in README and CONTRIBUTING) failed because `tool.uv.default-groups = ["dev"]` referenced a PEP 735 dependency group that did not exist. Fix: moved the dev dependency list from `[project.optional-dependencies]` to a `[dependency-groups]` table in `pyproject.toml` (the `providers` extra stays in optional-dependencies).
...
| CLI verification | Cross-artifact, review-policy, review-policy evidence-reference, evidence-capture, capture-lineage, readiness, and generic capability inspection passed |
...
| `src/mirage/runtime/evidence_capture_consistency.py` | Reference-only capture metadata, seal, review-policy evidence, and report-reference consistency assessment |

...
| M4, goal-to-evaluate workflow | Foundation implemented | Bounded review-only goal, steps, criteria, policy-bound checkpoint, and revalidation; no planning or execution pipeline |
...
| 3 | M4 goal-to-evaluate workflow | Review-only goal, steps, criteria, checkpoint, and revalidation exist, but no integrated engineering workflow exists | Context inputs, deterministic EIR-to-plan validation, evidence collection, evaluation, report artifacts, approval transitions, and integration tests |

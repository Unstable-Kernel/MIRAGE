...
| 3 | M4 goal-to-evaluate workflow | Review-only goal, EIR-bound steps, criteria, checkpoint, deterministic plan, evidence assessment, redacted context bundle, review trace, approval chain, advisory eligibility, persistence interface, revocation review, provenance seal, lifecycle validation, controlled context, and deterministic report artifact exist, but no integrated engineering workflow exists | Controlled real context inputs, real evidence collection, policy-bound evaluation, authenticated approval persistence and transitions, rendered report artifacts, and integration tests |
